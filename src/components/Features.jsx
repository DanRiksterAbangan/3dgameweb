...
const BentoTilt = ({children, className = ''}) => {
    ...
    const handleMouseMove = (e) => {
        ...
        const { left, top, width, height } = itemRef.current.getBoundingClientRect();  // Fixed typo here
    
        const relativeX = (e.clientX - left) / width;
        const relativeY = (e.clientY - top) / height; // Fixed: should be 'height' not 'width'
            ...
            <div className="grid h-[135vh] grid-cols-2 grid-rows-3 gap-7">
                ...
                <BentoTilt className="bento-tilt_2">
                    <div className="flex size-full flex-col justify-between bg-violet-300 p-5">
                        <h1 className="bento-title special-font max-w-64 !text-black text-xl sm:text-lg md:text-2xl lg:text-3xl">
                            More Coming Soon!
                        </h1>

                        <TiLocationArrow className="m-5 scale-[4] self-end text-black md:scale-[5] sm:scale-[3]" />
                    </div>
                </BentoTilt>
            </div>
        </div>
